...
func saveJob(db *sql.DB, message Message, status string, data string) error {
...
func getUnprocessedJobs(db *sql.DB) ([]Message, error) {
...
func httpWorker(id int, db *sql.DB, jobs <-chan Job, results chan<- Job, errors chan<- Message, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for job := range jobs {
		select {
		case <-ctx.Done():
			log.Printf("Worker %d encerrando devido ao contexto finalizado (tempo limite atingido)\n", id)
			errors <- Message{RecordID: job.RecordID, URL: job.URL}
...
func dbWorker(id int, db *sql.DB, jobs <-chan Job, results chan<- Job, errors chan<- Message, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for job := range jobs {
		select {
		case <-ctx.Done():
...
func processWorker(id int, db *sql.DB, jobs <-chan Job, errors chan<- Message, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for job := range jobs {
		select {
		case <-ctx.Done():
...
func lambdaHandler(ctx context.Context, db *sql.DB, messages []Message) ([]Message, []Message) {
	workerCount := getWorkerCount()

	httpJobs := make(chan Job)
	dbJobs := make(chan Job)
	processJobs := make(chan Job)
	errors := make(chan Message, len(messages))
